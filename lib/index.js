...
/**
 * Plugin config (read from the cordis config block):
 * - skillsDir: relative output root for forged SKILL.md files (default '.dsh-forge/skills')
 */
...
export function apply(ctx) {
  ...
  async function runForge(sid) {
    ...
    const outputDir = (ctx.config && ctx.config.skillsDir) || '.dsh-forge/skills'
  ...
  // Model-visible tool: the agent itself can trigger a forge.
  if (harness) {
    try {
      harness.registerTool(ctx, {
        name: 'forge_skill',
        description: 'Forge a reusable skill from a session trace: analyze the session event record, distill the successful tool sequence into SKILL.md (written under .dsh-forge/skills/) and register it via ctx.skills so every future session can load it. Omit sessionId to forge the most recently traced session.',
        parameters: {
          type: 'object',
          properties: {
            sessionId: { type: 'string', description: 'Optional exact session id to forge; omit to forge the most recent traced session with tool calls.' },
          },
        },
        output: {
          schema: {
          ...
          render(args, value) {
            if (value && value.error) return [{ type: 'text', text: 'Forge failed: ' + value.error }]
            if (value && value.name) return [{ type: 'text', text: 'Forged skill: ' + value.name + ' — ' + value.description }]
            return [{ type: 'text', text: 'Forge produced no result.' }]
          },
        },
        async execute(args, exec) {
          ...
          forged.push(res)
          return { name: res.name, description: res.description, from: res.from }
        },
      })
    } catch (e) {
      console.log('[dsh-forge] registerTool failed', e && e.message || e)
    }
  }
